...
export const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess }) => {
  const [roleTab, setRoleTab] = useState<'pelatih' | 'admin'>('pelatih');
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (found) {
      ...
      setErrorMsg(`Username atau Password ${roleTab === 'admin' ? 'Administrator' : 'Pelatih'} tidak sesuai.`);
    }
  };
        ...
        <form onSubmit={handleLogin} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1.5">
              Username {roleTab === 'admin' ? 'Admin' : 'Pelatih'}
            </label>
            <div className="relative">
              <input
                type="text"
                required
                placeholder={roleTab === 'admin' ? 'admin' : 'username pelatih'}
                value={username}
          ...
          >
            <span>Masuk Portal {roleTab === 'admin' ? 'Admin' : 'Pelatih'}</span>
